      ...
      <Container>
        ...
        <Navbar.Collapse id="basic-navbar-nav navbar_items">
          <Nav className="ms-auto header_link d-flex justify-content-center align-items-center">
            ...
            {loggedIn ? <button onClick={handleLogout}>logout</button> : <Nav.Link as={Link} to='/connexion'>
            ...
            {
              role === 'professionnel' ? <Nav.Link as={Link} to='/professionnel/dashboard' style={{ color: isHomeNav ? 'black' : '' }}><h6 className="text-white">Professionnel</h6></Nav.Link> : role === 'client' ? <Nav.Link as={Link} to='/client/dashboard' style={{ color: isHomeNav ? 'black' : '' }}><h6 className="text-white">Client</h6></Nav.Link> : null
            }
